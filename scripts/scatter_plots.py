...
from bokeh.models import (
    ...
    PrintfTickFormatter, ColumnDataSource, HoverTool,
    Span
)
...
def plot_vs_population(districts_budget_df):
    for q in districts_budget_df.budget_type.unique():
        ...
        source = ColumnDataSource(
            data=dict(x=df["num_total"] / 10000,
                    y=df["budget"] / 1000000,
                    desc=df['dname'])
        )

        p = figure(title = "", tools='hover,box_zoom,reset')
        vline = Span(location=df.num_total.mean() / 10000, dimension='height', line_color='gold', line_width=1.5)
        hline = Span(location=df["budget"].mean() / 1000000, dimension='width', line_color='gold', line_width=1.5)
...
def plot_vs_area(districts_budget_df):
    for q in districts_budget_df.budget_type.unique():
        ...
        source = ColumnDataSource(
            data=dict(x=df["AREA"] / 1000000,
                    y=df["budget"] / 1000000,
                    desc=df['dname'])
        )
        p = figure(title = "", tools='hover,box_zoom,reset')
        vline = Span(location=df.AREA.mean() / 1000000, dimension='height', line_color='gold', line_width=1.5)
        hline = Span(location=df["budget"].mean() / 1000000, dimension='width', line_color='gold', line_width=1.5)
...
if __name__ == "__main__":
    districts_budget_df = pd.read_csv('data/districts_budget.csv')[['dname', 'ประเภทแผนงาน', 'งบแผนงาน', 'AREA', 'num_male', 'num_female']]
    districts_budget_df['num_total'] = districts_budget_df.num_male + districts_budget_df.num_female
    districts = list(districts_budget_df.dname.unique())
    types = list(districts_budget_df['ประเภทแผนงาน'].unique())

    districts_budget_df.columns = ['dname', 'budget_type', 'budget', 'AREA', 'num_male', 'num_female', 'num_total']
    color_mapper = LinearColorMapper(palette=Oranges256[::-1], low=min(df.budget), high=max(df.budget))

    q_map = {'ทั่วไป/บริหาร/อื่นๆ': 'gen',
    'การคลัง': 'treasury',
